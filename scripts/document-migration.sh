#!/bin/bash
# Usage: ./scripts/document-migration.sh 002_add_contract_pii_mapping.sql

MIGRATION_FILE=$1

if [ -z "$MIGRATION_FILE" ]; then
  echo "Usage: ./scripts/document-migration.sh <migration_file>"
  echo "Example: ./scripts/document-migration.sh database/migrations/002_add_contract_pii_mapping.sql"
  exit 1
fi
