...
class Transformer:
    ...
    def _transform_record(
        ...
        canonical = {
            ...
            'reading_interval': (
                ...
                if record.get('interval_seconds') is not None
                else record.get('interval', default_interval)
            ),
        ...
        energy_value, energy_field = self._extract_numeric_with_field(record, mapping.get('energy_wh', []))
        canonical['energy_wh'] = energy_value
        ...
        if canonical['energy_wh'] is not None:
            ...
            if self._is_likely_kwh(record, source_type, matched_field=energy_field):
    ...
    def _extract_numeric(self, record: Dict, field_aliases: List[str]) -> Optional[Decimal]:
        ...
        result, _ = self._extract_numeric_with_field(record, field_aliases)
        return result

    def _extract_numeric_with_field(self, record: Dict, field_aliases: List[str]) -> Tuple[Optional[Decimal], Optional[str]]:
        """Extract numeric value from record, returning (value, matched_field_name)."""
        for alias in field_aliases:
            if alias in record and record[alias] is not None:
                value = record[alias]

                if isinstance(value, (int, float)):
                    return Decimal(str(value)), alias

                if isinstance(value, str):
                    try:
                        cleaned = value.replace(',', '')
                        return Decimal(cleaned), alias
                    except:
                        continue
    ...
    def _is_likely_kwh(self, record: Dict, source_type: str, matched_field: Optional[str] = None) -> bool:
        """
        Determine if energy values are likely in kWh instead of Wh.

        Priority:
        1. Explicit `unit` field in record always wins
        2. Field name ending in `_wh` → data is already Wh, no conversion
        3. Source-type heuristic fallback for ambiguous field names
        """
        # 1. Explicit unit field takes priority
        unit = record.get('unit', '').lower()
        if 'kwh' in unit:
            return True
        if unit and 'wh' in unit and 'k' not in unit:
            return False

        # 2. Field name explicitly says Wh — no conversion needed
        if matched_field and matched_field.endswith('_wh'):
            return False

        # 3. Source-type heuristic for ambiguous field names (e.g. "energy")
        kwh_sources = ['snowflake', 'manual']
        return source_type in kwh_sources
