...
|-------|-------|------------|
| **Phase 1** | Foundation | S3 bucket, Validator Lambda, DB schema, manual upload |
| **Phase 2** | API Key Inverters | SolarEdge + GoodWe fetchers, credential storage |
| **Phase 3** | OAuth Inverters | OAuth callback endpoint, Enphase + SMA fetchers |
| **Phase 4** | Client Platforms | Snowflake integration, documentation for clients |
| **Phase 5** | Polish | Monitoring, alerting, credential health checks |
...
│   ├── solaredge_fetcher.py         # SolarEdge API fetcher
│   ├── enphase_fetcher.py           # Enphase API fetcher
│   ├── goodwe_fetcher.py            # GoodWe API fetcher
│   └── sma_fetcher.py               # SMA API fetcher
│
├── lambdas/
│   └── validator/
...
│
└── docs/
    ├── SNOWFLAKE_INTEGRATION.md     # Instructions for clients
    └── FILE_FORMAT_SPEC.md          # Expected file formats
```
