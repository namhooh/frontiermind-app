...
DROP FUNCTION IF EXISTS get_contract_clause_stats(BIGINT);
DROP FUNCTION IF EXISTS get_clauses_needing_review(NUMERIC, INTEGER);

-- Drop indexes
DROP INDEX IF EXISTS idx_clause_beneficiary_party;
DROP INDEX IF EXISTS idx_clause_confidence_score;

-- Drop constraint
ALTER TABLE clause DROP CONSTRAINT IF EXISTS clause_confidence_score_check;

-- Drop columns
ALTER TABLE clause DROP COLUMN IF EXISTS confidence_score;
ALTER TABLE clause DROP COLUMN IF EXISTS beneficiary_party;
ALTER TABLE clause DROP COLUMN IF EXISTS summary;
